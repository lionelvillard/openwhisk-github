 ...
 * Edit a hook.
 * @method
 * @name #patchReposByOwnerByRepoHooksByHookId
 * @param {string} owner - Name of repository owner.
 * @param {string} repo - Name of repository.
 * @param {integer} hookId - Id of hook.
...
function main(args) {
    return new Promise((resolve, reject) => {
        ...
        if (args['hookId'] === undefined) {
            reject({
                error: 'Missing required integer parameter: hookId'
            });
            return;
        }

        path = path.replace('{hookId}', args['hookId']);
