 ...
 * @param {object} body - 
 * @param {string} [body.billing_email] - Billing email address. This address is not publicized.
 * @param {string} [body.company] - 
 * @param {string} [body.email] - Publicly visible email address.
...
function main(args) {
    return new Promise((resolve, reject) => {
        ...
        if (args['org'] === undefined) {
            reject({
                error: 'Missing required string parameter: org'
            });
            return;
        }

        path = path.replace('{org}', args['org']);
