 ...
 * Compare two commits
 * @method
 * @name #getReposByOwnerByRepoCompareByBaseIdHeadId
...
function main(args) {
    return new Promise((resolve, reject) => {
        ...
        if (args['baseId'] === undefined) {
            reject({
                error: 'Missing required string parameter: baseId'
            });
            return;
        }

        path = path.replace('{baseId}', args['baseId']);

        if (args['headId'] === undefined) {
            reject({
                error: 'Missing required string parameter: headId'
            });
            return;
        }

        path = path.replace('{headId}', args['headId']);
